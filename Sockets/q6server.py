...
data1 = connection_socket.recv(1024)
client_name = data1.decode()
send_msg = "Connection Recieved, " + client_name 
...
while(True):
    try:
        ...
        data3 = (data2.decode()).split()

        if data3[0]=="q":
            
            send_msg = "Session Ended"
            connection_socket.send(send_msg.encode('utf-8'))
            # close the connection
            connection_socket.close()
            
        elif data3[1]=="+":
            ans = float(data3[0])+float(data3[2])
